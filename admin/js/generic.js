...
$(function() {
    ...
    $(".to-slug").blur(function() {
        if($(this).hasClass("mode-add")) {
    ...
    $(".confirm-modal").click(function() {
        $("#modal-go").attr("href", $(this).data("href"));
    });
    ...
    function addAttr() {
        t_attr = attr.replace("#order#", $("#attr-holder li").length + 1);
        $(t_attr).appendTo("#attr-holder");
        $("select.chzn-select").select2();
        $("select.select-attribute").bind("change", function(){
            loadAttrValues($(this));
        });
        $(".remove-attribute").bind("click", function(){
            $(this).parent().parent().remove();
        });
        bindAddAttr();
    }
    
    function bindAddAttr() {
        l = $("#attr-holder").find("input").length;
        el = $("#attr-holder").find("input").eq(l - 1);
        el.bind('keydown', function(e){
            ...
            if (keyCode == 9) { 
                ...
                $(this).unbind('keydown');
                l = $("#attr-holder").find("input").length;
                el = $("#attr-holder").find("input").eq(l - 2).focus();
            }
        });
    }
    ...
    $("select.select-attribute").change(function(){
        loadAttrValues($(this));
    });

    function loadAttrValues(element) {
        id = element.val();
        to = element.parent().parent().find($("select.select-value"));
        to.attr("disabled", "disabled");
        to.html("<option>Carregando...</option>");
        to.select2();
        $.ajax({
            dataType: "json",
            url: "?a=attribute_values&as=json",
            success: function(data){
                html = '<option value="0">Selecione...</option>';
                i = 0;
                $.each(data, function(e){
                    if(data[e].attribute_id == id) {
                        i++;
                        html = html + '<option value="' + data[e].id + '">' + data[e].name + '</option>';
                    }
                });
                ...
                if(i === 0) {
                    to.html('<option>Nenhum valor encontrado</option>');
                } else {
                    to.html(html);
...
//********
    ...
    $(".dTable").dataTable({
        ...
        oLanguage: {
            "sUrl": "js/vendor/datatable.portuguese.txt"
        }
    });
...
$(document).ready(function(){
    $("#attr-holder").sortable({
        'handle' : '.handle',
         stop: function(){
             $.each($("#attr-holder li"), function(){
                 $(this).find($(".list_order")).val($(this).index()+1);
             });
         }
    });
    
    
    $("#add-seal").click(function(){
        id = $("#seal").val();
        if($("#seals-ul").find($("#li-"+id)).length === 0 && id != "") {
            img = $("#seal").find("option:selected").data("url");
            text = $("#seal").find("option:selected").text();
            seal = '<li id="li-'+id+'"><img src="'+img+'" />'+text+'<input type="hidden" name="seal[]" value="'+id+'" /><input type="hidden" class="list_order" name="seal_order[]" value="'+($("#seals-ul").find($("li")).length +1)+'" /><a href="javascript: void(0);" class="remove-seal" onclick="removeSeal(this);"><i class="icon-remove"></i></a></li>';
            $(seal).appendTo("#seals-ul");
        }
    });

    $("#seals-ul").sortable({
        stop: function(){
             $.each($("#seals-ul li"), function(){
                 $(this).find($(".list_order")).val($(this).index()+1);
             });
         }
    });
...
function fileToggle(el) {
    t_el = $(el).parent().parent().parent().find($(".input-file-holder"));
    if($(el).is(":checked")) {
        t_el.show();
    } else {
        t_el.hide();
