...
foreach($ProductLines as $Row) {
    $Products = $Product->getAll($Product->Table.".status = '1' AND {$Product->Table}.line_id = '".$Row->id."' AND product_subcategory_id = '".$attrs[2]['id']."' AND ambient = '$am'", "ORDER BY display_order ASC");
    if($Products) {
        if($i > 0) {
